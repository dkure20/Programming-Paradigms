 ...
 */
bool isTerminal(string result){
  if(result[0]=='<' && result[result.size()-1]=='>') return false;
  return true;
}


void helperFn(map<string, Definition>& grammar, vector<string>& res, string cur){
  if(isTerminal(cur)){
    res.push_back(cur);
  }else{
    Definition defin = grammar[cur];
    Production prod = defin.getRandomProduction();
      for (Production::iterator curr = prod.begin(); curr != prod.end(); curr++) {
          helperFn(grammar, res, *curr);
      }
  }
}
...
int main(int argc, char *argv[])
{
  ...
  for(int i = 0; i < 3; i++){
    ...
    helperFn(grammar, res, startWord);

          ...
          int numWorkers)
{
  ...
  pthread_mutex_init(&bank->checkingFor,NULL);
  sem_init(&(bank->nextDayStart),0,0);
  sem_init(&(bank->reportTransfer),0,1);
  if (bank == NULL) {
    return bank;
  }
...
Bank_Balance(Bank *bank, AccountAmount *balance)
{
  ...
  for(int i=0; i<bank->numberBranches; i++){
    sem_wait(&(bank->branches[i].lockToBranch));
  }
  ...
  for (unsigned int branch = 0; branch < bank->numberBranches; branch++) {
    ...
    if (err < 0) {
      for(int i=0; i<bank->numberBranches; i++){
      sem_post(&(bank->branches[i].lockToBranch));
      }
      return err;
    }

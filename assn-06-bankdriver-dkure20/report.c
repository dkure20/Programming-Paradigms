...
{
  pthread_mutex_lock(&bank->checkingFor);
  bank->numWorkerHasToFinished--;
  if(bank->numWorkerHasToFinished != 0){
    pthread_mutex_unlock(&bank->checkingFor);
    sem_wait(&(bank->nextDayStart));
  }else{
  Report *rpt = bank->report;
  assert(rpt); Y;
  ...
  if (rpt->numReports >= MAX_NUM_REPORTS) {
      // We've run out of report storage for the bank
      bank->numWorkerHasToFinished = numWorkers;
      pthread_mutex_unlock(&bank->checkingFor);
      return -1;
  }

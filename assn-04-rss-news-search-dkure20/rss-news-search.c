...
static void ImpelementStopWords(hashset *s);
static void BuildIndices(const char *feedsFileName, hashset* stopWords, hashset* seenUrls,  hashset* indices);
static void ProcessFeed(const char *remoteDocumentName, hashset* stopWords, hashset* indices, hashset* seenUrls);
static void PullAllNewsIndexs(FILE *dataStream, hashset* stopWords, hashset* indices, hashset* seenUrls);
static bool GetNextIndexTag(streamtokenizer *st);
static void ProcessSingleNewsIndex(streamtokenizer *st, hashset* stopWords, hashset* indices, hashset* seenUrls);
static void ExtractElement(streamtokenizer *st, const char *htmlTag,char dataBuffer[], int bufferLength);
static void ParseArticle(const char *articlename, const char *articleDescription, 
                        const char *articleURL,hashset *stopWords, hashset *indices, hashset* seenUrls);
static void ScanArticle(streamtokenizer *st, const char *articleTitle,const char *unused, const char *articleURL,hashset* stopWords, hashset* seenUrls, hashset* indices);
static void QueryIndices(hashset *stopWords,  hashset* indices);
static void ProcessResponse(const char *word, hashset* stopWords, hashset* indices);
static bool WordIsWellFormed(const char *word);
static bool alreadySeen(const char *articlename, const char *articleURL, hashset* seenUrls);
...
static int IndexHashFn(const void *elemAddr, int numBuckets)  {    
  Index* word = (Index*) elemAddr;
  return StringHash(&word->word, numBuckets);                                
}
static void IndexFreeFn(void *elemAddr) {
    free(((Index*)elemAddr)->word);
    VectorDispose(((Index*)elemAddr)->articles);
    free(((Index*)elemAddr)->articles);
}

static int IndexCompareFn(const void *elemAddr1, const void *elemAddr2) {
  return strcasecmp(((Index*) elemAddr1)->word, ((Index*) elemAddr2)->word);
}


static int ArticlesWordsCompareFn(const void *elemAddr1, const void *elemAddr2) {
  return ((myArticle*)elemAddr2)->words - ((myArticle*)elemAddr1)->words;
}
static void SortMapFn(void *elemAddr, void *auxData) {
  Index* newAddr = (Index*) elemAddr;
  VectorSort(newAddr->articles, ArticlesWordsCompareFn);
}

static int ArticleCompareFn(const void* elemAddr1, const void *elemAddr2) {
  myArticle* ar1 = (myArticle*) elemAddr1;
  myArticle* ar2 = (myArticle*) elemAddr2;
  if (strcasecmp(ar1->url, ar2->url) == 0) return 0;
  if (strcasecmp(ar1->name, ar2->name) == 0) return 0;
  return 1;
}

static void ArticleFreeFn(void *elemAddr) {
  myArticle *ar = (myArticle*) elemAddr;
  free(ar->name);
  free(ar->url);
}
...
int main(int argc, char **argv) {
  hashset stopWords; 
  ImpelementStopWords(&stopWords);
  hashset seenUrls;
  HashSetNew(&seenUrls, sizeof(char *), 1009, StringHash, StringCompare, StringFree);
  hashset indices;
  HashSetNew(&indices, sizeof(Index), 10007, IndexHashFn, IndexCompareFn, IndexFreeFn);
  setbuf(stdout, NULL);
  curl_global_init(CURL_GLOBAL_DEFAULT);
  Welcome(kWelcomeTextFile);
  BuildIndices((argc == 1) ? kDefaultFeedsFile : argv[1], &stopWords, &seenUrls, &indices);
...
static void ImpelementStopWords(hashset *stopWords) {
  HashSetNew(stopWords,sizeof(char*),1009, StringHash,StringCompare,StringFree);
    ...
    STNew(&st,infile,kNewLineDelimiters,true);
    while(STNextToken(&st,buffer,sizeof(buffer))){
      char* myStr = strdup(buffer);
      HashSetEnter(stopWords,&myStr);
    }
    STDispose(&st);
    fclose(infile);
}
...
static void BuildIndices(const char *feedsFileName, hashset* stopWords, hashset* seenUrl, hashset* words) {
  FILE *infile;
  streamtokenizer st;
  ...
  while (STSkipUntil(&st, ":") !=
    ...
    ProcessFeed(remoteFileName, stopWords, seenUrl, words);
  }
  HashSetMap(words, SortMapFn, NULL);
  STDispose(&st);
  fclose(infile);
  printf("\n");
}
...
static void ProcessFeedFromFile(char *fileName, hashset* stopWords, hashset* seenUrls, hashset* indices) {
  FILE *infile;
  streamtokenizer st;
  ...
  ScanArticle(&st, (const char *)fileName, articleDescription,
              (const char *)fileName, stopWords, seenUrls, indices);
  STDispose(&st); // remember that STDispose doesn't close the file, since STNew
                  // doesn't open one..
  fclose(infile);
}
...
static void ProcessFeed(const char *remoteDocumentName, hashset* stopWords, hashset* indices, hashset* seenUrls) {
  ...
  if (!strncmp(kFilePrefix, remoteDocumentName, strlen(kFilePrefix))) {
    ProcessFeedFromFile((char *)remoteDocumentName + strlen(kFilePrefix)
                        ,stopWords, indices, seenUrls);
    return;
  }

  FILE *tmpFeed = FetchURL(remoteDocumentName, "tmp_feed");
  PullAllNewsIndexs(tmpFeed, stopWords, indices, seenUrls);
  fclose(tmpFeed);
}
...
static void PullAllNewsIndexs(FILE *dataStream, hashset* stopWords, hashset* indices, hashset* seenUrls) {
  ...
  while (GetNextIndexTag(
      &st)) { // if true is returned, then assume that <Index ...> has just been
              // read and pulled from the data stream
    ProcessSingleNewsIndex(&st, stopWords, indices, seenUrls);
  }
...
static void ProcessSingleNewsIndex(streamtokenizer *st, hashset* stopWords,
                                   hashset* indices, hashset* seenUrls) {
  char htmlTag[1024];
  ...
  ParseArticle(articleTitle, articleDescription, articleURL, stopWords, indices, seenUrls);
}
...
static void ParseArticle(const char *articlename,const char *articleDescription,const char *articleURL,hashset *stopWords, 
                         hashset *indices,
                         hashset *seenUrls) {
...
static bool alreadySeen(const char *articlename, const char *articleURL, hashset* seenUrls) {
  ...
  char* name = strdup(articlename);
  bool alreadyseen = true;
  if (HashSetLookup(seenUrls, &url) != NULL || HashSetLookup(seenUrls, &name) != NULL) {
    alreadyseen = false;
  } 
  free(url);
  free(name);
...
static void ScanArticle(streamtokenizer *st, const char *articleTitle, const char *unused,const char *articleURL, hashset* stopWords,
                        hashset* seenUrls, 
                        hashset* indices) {
  if (!alreadySeen(articleTitle, articleURL, seenUrls)) // Check if we already scanned
    return;
  ...
  while (STNextToken(st, word, sizeof(word))) {
    if (strcasecmp(word, "<") == 0) {
      ...
      if (WordIsWellFormed(word) && HashSetLookup(stopWords, &newWord) == NULL) {
        numWords++;
        Index* ind = HashSetLookup(indices, &newWord);
        if (ind != NULL) {
           myArticle article;
            article.name = strdup(articleTitle);
            article.url = strdup(articleURL);
            int idx = VectorSearch(ind->articles, &article, ArticleCompareFn, 0, false);
            if (idx != -1) {
              ((myArticle*)VectorNth(ind->articles, idx))->words++;
            } else {
              myArticle art;
              art.name = strdup(articleTitle);
              art.url = strdup(articleURL);
              art.words = 1;
              VectorAppend(ind->articles, &art);
            }
            free(article.name);
            free(article.url);
        } else {
          vector *newArticle = malloc(sizeof(vector));
          VectorNew(newArticle, sizeof(myArticle), ArticleFreeFn, 1007);
          myArticle article;
          article.name = strdup(articleTitle);
          article.url = strdup(articleURL);
          article.words = 1;
          VectorAppend(newArticle, &article);
          Index ind;
          ind.word = strdup(word);
          ind.articles = newArticle; 
          HashSetEnter(indices, &ind);
        }
...
static void QueryIndices(hashset* stopWords, hashset* indices) {
  ...
  while (true) {
    ...
    if (strcasecmp(response, "") == 0)
      break;
    ProcessResponse(response, stopWords, indices);
  }
}
int min(int a, int b){
  if(a<=b) return a;
 ...
 */
void printResults(const char* word, hashset* indices,Index* ind){
        int articleNum = VectorLength(ind->articles);
        int minResult = min(articleNum,10);
        for (int i = 0; i < minResult; i++) {
          myArticle* currArticle = VectorNth(ind->articles, i);
          printf("%d.) \"%s\" [search term occurs %d %s]\n\"%s\"\n", 
                  (i + 1), currArticle->name, 
                  currArticle->words, 
                  (currArticle->words == 1 ? "time" : "times"), 
                  currArticle->url);
        }
}
static void ProcessResponse(const char *word, hashset* stopWords, hashset* indices) {
  if (WordIsWellFormed(word)) {
   if (HashSetLookup(stopWords, &word) != NULL) {
      ...
      Index* ind = HashSetLookup(indices, &word);
      if (ind != NULL) {
        printResults(word,indices,ind);
      } else {

...
;;
(define (intersection-points circlelst)
    (if (< (length circlelst) 2) '()
      (append (intersect(car circlelst) (cadr circlelst)) 
      (intersection-points (cons (car circlelst) (cddr circlelst)))
      (intersection-points (cons (cadr circlelst) (cddr circlelst)))
      )
    )
)
(define (distance-product pt ptlist)
  (apply * (map (lambda (point) (dist pt point))
    (remove pt ptlist))
  )
)
(define (rate-points pointlst)
    (map (lambda (point)
      (list (distance-product point pointlst) point) )
      pointlst
    )
)
(define (sort-points rated-pts)
  (quicksort rated-pts (lambda (firstLs secondLs) (< (car firstLs) (car secondLs)))))

(define (clumped-points pointlst)
  (map cadr (prefix-of-list(sort-points(rate-points pointlst)) (quotient (length pointlst) 2)))
)
(define (average-point pointlst)
  (let ((avgpt (make-pt (/ (apply + (map x pointlst)) (length pointlst))
      (/ (apply + (map y pointlst)) (length pointlst)))))
      (list (distance-product avgpt pointlst) avgpt))
)

(define (best-estimate circlelst)
  (average-point (clumped-points (intersection-points circlelst)))
)
(define (where-am-i distances starpoints)
  (sort-points (map best-estimate (all-guesses distances starpoints)))
)
